...
def build() -> list[tuple[str, str]]:
    ...
    corrupt_payload = b64url(b"{not valid json at all")
    corrupt = header + "." + corrupt_payload + ".c2lnbmF0dXJl"
    tokens.append(("corrupt payload, not valid JSON", corrupt))
...
def main() -> None:
    ...
    out = os.path.join(here, "tokens.txt")
    tokens = build()
    lines = [
        "# tokenscope test vectors, generated by samples/make_tokens.py",
        "# signing secret is the published RFC 7515 A.1 style test secret",
        "# reference time NOW = %d (2026-01-01T00:00:00Z)" % NOW,
    ]
    ...
    with open(out, "w", encoding="utf-8", newline="\n") as handle:
        handle.write("\n".join(lines) + "\n")
    print("wrote %s with %d tokens" % (out, len(tokens)))
